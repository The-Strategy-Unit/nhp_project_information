...
For each local authority district (LAD) which contributes to the caseload of the trust, we look at that caseload as a proportion of all the given LAD's caseload at all hospitals. We then apply that proportion to the LAD's total population in order to arrive at a proportion of the LAD's population which is considered to be in the catchment of the given trust. These proportions are then used as weights to calculate the total catchment population of the trust from all the LADs which contribute to its caseload. 
