...
This note sets out the current approach to non-demographic growth (NDG) for the NHP demand and capacity (D&C) model whilst consideration is given to setting a revised national assumption. The approach is expected to change once that assumption is set. The note explains the current use of two variants for urgent and emergency care IP non-demographic growth. 
