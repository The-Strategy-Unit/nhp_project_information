...
This functionality is not currently available to all users. Information provided here is intended to support local discussions around setting inequalities parameters. If you have further questions, please [contact us](../contact.qmd).
...
data <- data.frame(
  quintile = seq(1, 5, 1),
  population = c(154565, 84885, 69171, 77929, 45067),
  activity_count = c(122, 96, 109, 150, 88)
) |>
...
plot <- ggplot(data, aes(x = as.factor(quintile), y = activity_rate)) +
  ...
  geom_smooth(
    aes(x = as.numeric(quintile), y = activity_rate),
    method = "lm",
    colour = "orange",
    linetype = "dashed",
    se = FALSE
...
table <- data.frame(
  "IMD Quintile" = c("1", "2", "3", "4", "5"),
  "Observed activity rate" = c(
    data$activity_rate[data$quintile == min(data$quintile)],
  ...
  "Fitted Rate" = c(
    predict(model, newdata = data.frame(quintile = 1)),
  ...
  "Level-up Target Rate" = c(
    rep(predict(model, newdata = data.frame(quintile = 5)), 4),
    data$activity_rate[data$quintile == 5]
  ),
  "Level-up Multiplier" = c(
    predict(model, newdata = data.frame(quintile = 5)) /
  ...
  "Level-down Target Rate" = c(
    rep(predict(model, newdata = data.frame(quintile = 1)), 4),
    data$activity_rate[data$quintile == 1]
  ),
  "Level-down Multiplier" = c(
    1,
    predict(model, newdata = data.frame(quintile = 1)) /
      data$activity_rate[data$quintile == "2"],
    predict(model, newdata = data.frame(quintile = 1)) /
  ...
  "Zero-sum Target Rate" = rep(mean(data$activity_rate), 5),
    ...
    mean(data$activity_rate) / data$activity_rate[data$quintile == 1],
...
A <- plot +
  ...
  geom_segment(
    data = subset(data, quintile != 5),
    aes(
      x = as.factor(quintile),
      y = activity_rate,
