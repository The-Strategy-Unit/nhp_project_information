...
3. Having established, for each change factor, the characteristics of the spell that determine which user parameter applies, the parameters that have been set by the user are retrieved.  The parameters may be point estimates, or the parameters might describe a distribution reflecting the user's uncertainty about the extent to which this change factor might apply.  In most cases users are asked to pass their 80% confidence interval for the change factor and the distribution of the change factor parameter is assumed to be Normal. 

In our example, let's assume that the user's parameters for the repatriation/expatriation parameters (80% CI for a Normal distribution), are 0.91 and 0.99. This would indicate that the user believes that there is a 10% chance that more than 9% of non-local emergency admissions to trauma & orthopaedics would be expatriated in future, and a 10% chance that fewer than 1% of such cases will be expatriated.  From this information a Normal distribution is constructed. Similar distributions are constructed for each of the applicable change factors.  Note that special rules apply to derive distributions for the demographic change factors.

4. Next we randomly sample a single value from these distributions.  Let's assume that sampled values are 0.96, 1.26, 1.20 and 0.79 for the repatriation/expatriation, demographic, non-demographic and activity mitigator change factors respectively.  [Note that the process by which the demographic parameter is obtained is somewhat more involved].

5. We then calculate the product of these sampled parameter values as our summary parameter.  So, in our example, the summary parameter would be 1.15 (= 0.96 x 1.26 x 1.20 x 0.79)

6. We then randomly sample from a Poisson distribution with rate, $\lambda$ (lambda), set to our summary parameter, 1.15.  The result of this random sample will be an integer value.  Let's assume that in the case the random sample produces the value 2.

7. We replicate the record with this frequency.  So the record would be represented twice in our modelled future year.
