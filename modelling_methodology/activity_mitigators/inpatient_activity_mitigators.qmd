---
title: Inpatient-Related Types of Potentially Mitigatable Activity (TPMAs)
...
The [Ambulatory Emergency Care Network][2] has published *Directory of Ambulatory Emergency Care for adults, 6th ed* (2018) which identifies emergency conditions that have the potential to be managed as an emergency day case delivered in an SDEC unit. We have grouped these conditions into four TPMA cohorts:
...
The spells identified by these TPMAs exclude any with a length of stay (LOS) of 3 days and over, as these spells are considered less likely to be treatable within a day. This TPMA allows Trusts to make an assumption about what proportion of the identified spells they think will, in future, be delivered within an SDEC facility and not as an admitted patient. The model removes this quantity of spells from the inpatients data and adds them to the A&E outputs as type 5 (SDEC) attendances, enabling clear identification and quantification of future SDEC activity with the model outputs.

[2]: https://www.nhselect.nhs.uk/improvement-collaboratives/Clinical-networks/AEC

NOTE: these TPMAs replace the previous set of Ambulatory Emergency Care (AEC) efficiency TPMAs that adjusted the LOS of selected spells to 0 LOS but kept them within the inpatients (IP) dataset. The change reflects the new requirement for Trusts to record SDEC activity within the ECDS dataset.
...
This TPMA does not impact readmissions where the first admission was flagged as a [Well baby](/data_extraction/data_log.qmd#version-1.1).
...
This activity avoidance TPMA identifies patients who may be suitable for admission to an ARI or Heart Failure virtual ward. 
...
Historically there has been a sustained and continuing reduction in average length of stay (LoS). The model allows users to make assumptions about future reductions in LoS relating to some specific activity cohorts, such as patients aged 75 and over who are admitted as an emergency, but it is recognised that continued LoS reductions may also continue to be achieved for patients outside of these specific cohorts. This TPMA therefore allows users to set assumptions about the expected reduction in the average LoS for emergency and elective spells that are not covered by the other specific efficiency TPMAs. The assumed % reduction is applied to the average LoS.
...
This efficiency TPMA identifies patients who may be suitable for earlier discharge through admission to step down ARI or Heart Failure virtual wards. 
...
We have counted all activity in 2023/24 where the primary procedure code is included in the [`has_procedure` logic](/data_extraction/inpatients.html#has_procedure).
...
As a data quality step, if a procedure has a total less than 100 (in the entirety of England in 2023/24) then we remove this procedure from our analysis.
