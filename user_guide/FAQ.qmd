...
## I have sign off to use our own custom population projections - how do I submit these to you?

Once you have all relevant sign-offs, we can accept your population projections 
provided it is delivered to us in a specific format. Please see [this csv example](../resources/national_demographic_factors.csv) - please structure your data with the same layout, headings and file format.

A custom birth projection is also required. You may select either of the two options below:

1. Use our methodology to create custom birth projections from custom population projection you have submitted, or
2. Provide us with a custom birth projection in the format of this [national birth factors csv](../resources/national_birth_factors.csv).

You can [contact the inbox](mailto:mlcsu.su.datascience@nhs.net) for questions or to submit your file.

## What happened to the request I made to change/add functionality to the model or its apps?

The NHP demand model and its associated tools have many stakeholders and user groups.
We aim to create and maintain data science tools that deliver the maximal value
to the maximal number of users who can create patient care impact using our
outputs.
When a request or suggestion is proposed, it is triaged by our product team to
estimate the user value, and all potential pros and cons of doing the work.
We may decide that we will not progress the request any further because, for
example;

- User need is not proven or cannot be validated
- The change would represent an unacceptable increase in maintenance costs
- The change would not be possible using our current infrastructure
- The suggested change would introduce governance problems like IG issues

Should we agree that the request is valid, useful and theoretically possible to
implement, it will be prioritised against our existing backlog of work, and we will
implement the change once its priority is ranked high enough relative to other work.

If you would like to ask us about a specific request or suggestion you have
submitted or discussed in the past, feel free to email the product team (please
add 'FAO Product Team' to the subject bar) at
[The Strategy Unit Data Science Team](mailto:mlcsu.su.datascience@nhs.net) and we
will investigate for you.

## Can we upgrade scenarios created on older versions of the model?

It is mandatory to use the latest version of the model. We do not support running older versions of the model.

Scenarios will automatically be upgraded to the latest version of the model, unless they meet any of the following criteria:

* The scenario was created with a baseline year before 2023/24
* The scenario was created with a model version before v1.2

We cannot transfer parameters from any scenarios meeting these criteria to the latest model version. Should users wish to rerun a scenario meeting these criteria, parameters will need to be re-entered manually using the inputs app.

Note also that any scenarios prior to v3.6 which are upgraded will have their population projection overwritten to "migration category" by default.

## What are TPMAs and how do they relate to 'mitigators'?

'Types of potentially mitigatable activity (TPMAs)' is the term now used to to describe the activity types for which model users can enter their assumptions around delivery in the horizon year. They were previously referred to as 'mitigators', but this terminology has been superseded and replaced with TPMAs to improve clarity and avoid confusion between the activity types referred to, and the mechanisms by which hospitals might attempt to reduce activity.
...
As an NHS team committed to open analysis, we have ensured that all of the code that can be open is currently open source. For more information on the code that is open and closed in the repository please go to the [components overview chapter](../project_plan_and_summary/components-overview.qmd).
...
We will continue to add in new baseline years to the model as they become available, yearly.
...
The model allows users to select their 'horizon year': the year they wish the model to forecast up to. We recommend that users select a horizon year at least 10 years beyond the opening of the new hospital. The model can forecast up to and including the 2047/48 financial year. The inputs app defaults to the 2041/42 financial year.

## Do the types of potentially mitigatable activity (TPMAs) account for virtual wards?

Yes, we introduced a new virtual wards TPMA in May 2024. Information about all the TPMAs is available [here](../modelling_methodology/activity_mitigators/inpatient_activity_mitigators.qmd).

## Which types of potentially mitigatable activity (TPMAs) should be prioritised when setting assumptions?

The 20 TPMAs associated with the highest levels of healthcare activity are listed in a table within the inputs app ('Summary table'). While all activity TPMAs should be considered, this table can help users prioritise which TPMAs to spend more time forecasting.

## Can a list of all the types of potentially mitigatable activity (TPMAs) be downloaded?

Yes, this is available from the [TPMAs lookup page](mitigators_lookup.qmd). Where needed, users can 'Print to PDF' any of the pages with activity TPMA information. This will include the charts showing trend/peer comparison information.

## What is the National Elicitation Exercise (NEE)?

Subject matter experts were invited to provide forecasts about future hospital activity using an evidence-based elicitation exercise. The methodology and outcome has been [published in an open-access journal](https://doi.org/10.1136/bmjopen-2024-084632). In the inputs app, the aggregated forecasts from the exercise are presented for 77 types of potentially mitigatable activity (TPMAs). They provide a useful 'outside' view but should not be interpreted as an expectation of what can be achieved locally.
...
The Trust peers displayed in the inputs app (in the comparative charts for the TPMAs) are derived from an earlier Excel version of this tool: [Trust Peer Finder Tool](https://app.powerbi.com/view?r=eyJrIjoiMjdiOWQ4YTktNmNiNC00MmIwLThjNzktNWVmMmJmMzllNmViIiwidCI6IjUwZjYwNzFmLWJiZmUtNDAxYS04ODAzLTY3Mzc0OGU2MjllMiIsImMiOjh9).
...
## Why can't we see a detailed breakdown of exactly which activity was avoided by each type of potentially mitigatable activity (TPMA)?
...
All of the activity avoidance TPMAs apply at the same time. If more than one activity avoidance TPMA applies to a row of data, then their values are multiplied together to produce an activity avoidance summary parameter. This activity avoidance summary parameter is then used as $p$ when we randomly sample from a $\text{Binomial}(n,p)$ distribution, where $n = 1$.

With the efficiencies TPMAs, if more than one efficiency TPMA applies to a row of data, the model randomly selects which TPMA to apply in each run. It then uses the sampled value for the efficiency TPMA as $p$ when sampling from a $\text{Binomial}(n,p)$ distribution, where $n = LOS$, the Length of Stay for the row.

It is therefore not possible to exactly state which TPMA has removed specific types of activity. We can provide high-level estimates of activity removed through the [step counts/impact of changes](glossary.qmd#impact-of-changes), and a breakdown of avoided activity by age and sex in the model results. We have not yet implemented the ability to see a breakdown of activity removed by the efficiency TPMAs in the model results.

## What effect does the v3.1 model methodology change have on model comparability? {#method-change}

An improvement was made to the model so that the outputs more accurately reflect the impact that each activity TPMA has had on reducing activity.

Model versions before Version 3.1 applied modifying factors to the baseline activity levels in a single step.

This means that, if a TPMA was expected to reduce one type of activity by, say, 10%, this represented a reduction of 10% of the baseline rate, rather than the inflated activity rates created by applying the growth factors (demographic and non-demographic growth).
...
The waterfall chart shows a "model interaction factor", which reflects only the interaction between factors in the first, "Initial Projection", stage. The degree of overlap of the TPMAs is not shown in the waterfall chart, but is included in the "activity avoidance" bar.

Additionally, the individual bar chart displaying the effect of each TPMA does not account for interaction effects, meaning the impact of each TPMA may be slightly overstated. However, the size of this interaction effect can be seen in the Excel download.
...
## What data sharing arrangements are in place?

Producing demand estimates within the work of the NHP demand and capacity project requires the use of row level pseudonymised data. This data is only accessed by specific MLCSU staff who need to use this data to carry out their work on the modelling. The data is stored on password protected NHS provisioned servers. This pseudonymised row level data is not shared with any third parties as part of the work of the modelling team.

The modelling work does generate data summaries that are safe to access by individuals outside the immediate team. Even pseudonymised data can be reidentified under the appropriate conditions and the dashboard and high level summaries which are provided on a server which is hosted on the open internet have appropriate suppression of small numbers (suppressing numbers <5) in line with NHS guidance to prevent reidentification.

The work does require sharing more detailed summaries with the hospital schemes themselves as well as named contractors commissioned by the programme within the 'Health Delivery Partnership'. These detailed summaries contain small numbers that are not suppressed but the risk of reidentification is highly mitigated as described in the DPIA. Both contractors operate under a contract which contains specific instructions around use of data including prohibiting sharing of any data. NHS staff in hospitals with whom the summaries are shared will be bound by their local information governance policies.

Data minimisation is used extensively to reduce the amount of information disclosed. All of the age data is in categories (0, 1-17, 18-65, 65+) and other than sex there is no other demographic information present. 

No other data should be shared as part of this work, neither data summaries with variables that increase the risk of reidentification (for example, date of birth or LSOA) nor with any other parties than those described above. All of this detail is held within a DPIA approved and held by NHS England.
