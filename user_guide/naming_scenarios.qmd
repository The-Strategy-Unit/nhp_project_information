...
In particular, we recommend that you give each model run a unique name, which will help prevent errors in communication or processing.
...
It's possible to select an existing model run from the inputs app and re-run it with tweaked parameters. This could result in model runs with the same name but different parameters, although they'll be differentiated in the outputs app by the date and time they were run. We suggest you create a new name if you've adjusted the parameters, which will help differentiate these runs.
