...
### Change to waterfall diagram methodology

The waterfall diagram, found on the impact of changes tab on the outputs dashboard, shows the impact of each of the individual components of the model which are increasing or decreasing the amount of activity that the model predicts at the horizon year. 

Previous versions of the waterfall diagram have included a ["model interaction term"](../user_guide/glossary.qmd#impact-of-changes) which shows the effects of compounding within the model. We now have the option to redistribute this interaction term amongst the other factors impacting the model results.
