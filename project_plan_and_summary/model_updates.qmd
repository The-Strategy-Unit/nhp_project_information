...
In order to improve these detailed outputs, we had to change some of the data underlying the model, adding an index to the OP and A&E data. This has impacted very slightly on the random sampling of the model. Any scenarios that were run between 2024/11/13 and 2024/11/27 will have very minor differences in OP and A&E results to scenarios run after 2024/11/27, even if using exactly the same parameters. These changes are less than 0.5% or less.
...
Note that the data in the inputs app has not had the bugfixes that the data in the model has. However, the level of difference is small and should not affect practical usage of the model, inputs and outputs apps. We are working to update this soon.
