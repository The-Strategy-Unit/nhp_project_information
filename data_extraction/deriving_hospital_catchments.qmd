...
In the model, we need to derive catchment populations to solve a couple of problems:

1. for the [demographics](/modelling_methodology/demographic_modelling/demographic_modelling.qmd) part of the model, we need to join provider based data to local authority based data
2. for the inputs app, we display some rates as "per 1,000 population", so we need a population to divide by

OHID previously (circa 2022) produced a tool ["NHS Acute (Hospital) Trust Catchment Populations"](https://app.powerbi.com/view?r=eyJrIjoiODZmNGQ0YzItZDAwZi00MzFiLWE4NzAtMzVmNTUwMThmMTVlIiwidCI6ImVlNGUxNDk5LTRhMzUtNGIyZS1hZDQ3LTVmM2NmOWRlODY2NiIsImMiOjh9). We have used the logic stated in this tool to recreate the catchments for hospital providers, based on acute inpatient activity. This allows us to assign a percentage of a local authority’s population, by single year of age and sex, to each provider, allowing us to create catchment populations by provider.

In order to do this, we need to derive the Local Authority 2023 code (`LAD23CD`), and then use this to join our inpatient activity to the population files for local authorities.

We use these catchments derived from inpatient activity for all other types of activity (e.g. inpatients, outpatients, A&E).
...
This is a many-one lookup, where each `LSOA21` is mapped to a single `LAD23CD`.
...
So, we can specifically filter out the rows that don't match this table above, which leaves us with a many-one mapping from `LSOA11` to `LAD23CD`.
...
1. they use 5 year age bands, we use single year ages. This is for two reasons. Firstly, we can re-use these catchments in the demand models demographic module, which needs single year ages. Secondly, when we use these for direct age/sex standardisation with the TPMAs, some of these are defined for specific age ranges. For example, the Alcohol Partially Attributable TPMAs are defined for 16yo+.
...
3. Create a percentage 1. / 2. (that is, for each subgroup `{age, sex, year, LAD}`, what percentage of patients went to each provider)
