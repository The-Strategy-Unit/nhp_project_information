...
The methodology is all laid out in the linked document. The Sql used is below (slightly modified because we have some additional linkage tables in our own datawarehouse that aren't standard):
...
FROM (
  SELECT
    i.EPIKEY,
    RN = ROW_NUMBER() OVER (
      PARTITION BY i.token_person_id, i.ADMIDATE, i.PROCODE3, ISNULL(i.PROVSPNOPS, i.PROVSPNO)
      ORDER BY i.EPISTART DESC, i.EPIORDER DESC, i.EPIEND DESC, i.EPIKEY DESC
    )
  FROM
    tbInpatients i
  WHERE
    i.EPISTAT = '3' -- Finished consultant episode
  AND
    i.CLASSPAT IN ('1', '2', '5')
) t
  WHERE
