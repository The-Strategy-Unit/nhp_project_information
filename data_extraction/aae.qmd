...
Data is sourced from Hospital Episode Statistics (HES) Accident and Emergency (AE) dataset.
We extract aggregrated attendance level data for a given year.
...
We select records where the `aeattendcat` field is `"1"` (First A&E attendance).
This filters out planned and unplanned follow-up's.
