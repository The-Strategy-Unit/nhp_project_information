...
HES includes data about NHS patients treated by independent sector providers. 
This includes admitted patient care, outpatient appointments, and A&E attendances.
This can sometimes be identified by an NHS trust provider code, but a site code for 
an independent sector hospital. 
...
- Filter out spells with duration > 2 years that are likely data quality errors, excluding:
   - mental health specialty spells (specialty codes starting with "7")
   - complex cases with many episodes (10 or more)

## Derived fields

### `group`

We mainly determine the group based on the `admimeth` field:

- if `admimeth` starts with a `"1"`, then `"elective"`
- if:
  - `admimeth` starts with a `"3"` or `tretspef` is `"501"` or `"560"`
  - and `sex` is `"2"`
  - and `age` is between `13` and `55`
  - then `"maternity"`
